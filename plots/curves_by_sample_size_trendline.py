...
def main():
    parser = get_argument_parser('curves_by_sample_size_trendline')
        ...
        for label in labels:
            ...
            for sample_size in sample_sizes:
                ...
                for idx in range(length):
                    ...
                    to_plot.append(float(Decimal(sum(vals)) / Decimal(len(vals))))

                # calculate the trendline
                z = np.polyfit(epochs[name], to_plot, 1)
                p = np.poly1d(z)
                pylab.plot(
                    epochs[name],
                    p(epochs[name]),
                ...
                plt.plot(epochs[name], to_plot, label=name, color=colors[sample_size], linewidth=2.5, alpha=0.5)
            ...
            ensure_outputdir_and_write_chart(args.path_to_output + '/curves_by_sample_size_trendline', plt, filename, dpi=300)
